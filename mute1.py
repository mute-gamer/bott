...
async def send_promotion():
    ...
    promo_message = f"{random.choice(messages)}\n\n👉 https://www.youtube.com/channel/{CHANNEL_ID}"
    
    await send_telegram_message(promo_message)
    
    current_time = time.time()
    if current_time - last_x_promotion_time >= 86400:  # 24 hours check
        send_twitter_post(promo_message)
